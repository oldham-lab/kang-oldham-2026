...
#################
# DE using DEseq2, Lein MTG
#################

# Set save dir
save_dir <- file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_s2/v1/jorstad_MTG/")
if(!dir.exists(save_dir)){dir.create(save_dir, recursive = T)}

# Load Jorstad MTG pseudobulk data
data_save_dir <- file.path(Sys.getenv("DATA_OTHER_DIR", "/home/gugene/data_other"), "lein_allRegions_pseudobulk_by_donor_all_genes/")
rawcounts <- fread(paste0(data_save_dir,"Lein_2023_cell_expression_by_donor_subclass_sum_MTG.csv"), data.table=F) |>
  tibble::column_to_rownames(var = "V1")
cell_anno_pb <- fread(paste0(data_save_dir,"Lein_2023_cell_annotations_by_donor_subclass_sum_MTG.csv"),data.table=F)
...
# Reload DE data (jorstad MTG)
edger <- readRDS(file.path(Sys.getenv("MEGASET_DIR", "/home/gugene/RNAseq_megaset"), "14-ESSA_data/lein2023/data/DE_gene_list_subclass_MTG_blockDonorSum_allgenes.RDS"))
reslist <- qread(file = file.path(Sys.getenv("MEGASET_DIR", "/home/gugene/RNAseq_megaset"), "14-ESSA_data/lein2023/DEseq2/DEseq2_lein_mtg_subclass.qs"))
reslist <- reslist[match(names(edger), names(reslist))]

# # Calculate FDR across all cts (DEseq2)
# reslist <- mapply(\(x,y){
#   x$ct <- y
#   x$gene <- rownames(x)
#   return(x)
# }, reslist, names(reslist), SIMPLIFY = F)
# resdfall <- do.call(rbind, reslist) |>
#   dplyr::filter(!is.na(pvalue)) |>
#   mutate(qvalue = qvalue::qvalue(resdfall$pvalue)$qvalue)
# # length(unique(resdfall$gene[resdfall$qvalue < 0.05]))
# # 17149
# # length(unique(resdfall$gene[resdfall$padj < 0.05]))
# # 17180
# # table(resdfall$ct[resdfall$qvalue < 0.05])
# # table(resdfall$ct[resdfall$padj < 0.05])
# # bla1 <- unique(resdfall$gene[resdfall$qvalue < 0.05])
# # bla2 <- unique(resdfall$gene[resdfall$padj < 0.05])
# # # > length(intersect(bla1, bla2))
# # # [1] 16585
# # # not much difference between genes from built-in padj and qvalue()

# # Calculate FDR across all cts (edger)
# edgerall <- do.call(rbind, edger) |>
#   dplyr::filter(!is.na(PValue)) |>
#   mutate(qvalue = qvalue::qvalue(PValue)$qvalue)
# # length(unique(edgerall$gene[edgerall$qvalue < 0.05]))
# # [1] 15811
# # length(unique(edgerall$gene[edgerall$PValue < 0.05]))
# # [1] 19997

# Gather unique FDR genes
# # qvalue
# uniquegenes <- list()
# for(i in seq_along(cts)){
#   genesin <- resdfall |> 
#     dplyr::filter(ct == cts[i],
#                   qvalue < 0.05) 
#   genesout <- resdfall |> 
#     dplyr::filter(ct != cts[i],
#                   qvalue < 0.05)   
#   uniquegenes[[i]] <- genesin$gene[!genesin$gene %in% genesout$gene]
# }
# # > unlist(lapply(uniquegenes,length))
# #  [1] 811  74 174 135 156 133  47 180 142  57 190 144 117  85 886 258 932  49 208
# # [20]  67 103  97  75 149
# # > sum(unlist(lapply(uniquegenes,length)))
# # [1] 5269
#  padj
uniquegenes <- list()
for(i in seq_along(cts)){
...
# > unlist(lapply(uniquegenes,length))
#  [1]  836   32  140   77   83   84   16  117   66   23  142   98   72   30  842
# [16]  206 1043   23  131   40   42   59   58   75
# > sum(unlist(lapply(uniquegenes,length)))
# [1] 4335

# Gather DE genes (padj)
...
# ggsave(p, file = file.path(save_dir, "/DEgene_meanExpr_violin.pdf"), width = 4, height = 2.5)
...
p <- ggplot(plotdf_mean, aes(x = which, y = means, fill = which, color = which)) + 
  ...
  scale_fill_manual(values = c("grey", paletteer::paletteer_d("khroma::highcontrast"))) +
  scale_color_manual(values = c("grey", paletteer::paletteer_d("khroma::highcontrast")))
ggsave(p, file = file.path(save_dir, "DEgene_meanExpr_boxjitter.pdf"), width = 3, height = 2.5)
...
p2 <- ggplot(plotdf_sanity, aes(x = type, y = val, fill = type)) +
  ...
  theme(text = element_text(size = 12),
        legend.position = "none")  +
  scale_fill_manual(values = c("grey", paletteer::paletteer_d("khroma::highcontrast")))
ggsave(p2, file = file.path(save_dir, "DEgene_meanExpr_sanity.pdf"), width = 3, height = 2)

# # Plot pctg of genes above sanity threshold (for matching DE genes only)
# elrl <- mapply(\(x, y){
#   intersect(x, y)
# }, el, rl, SIMPLIFY = F)

# plotdf_sanity <- lapply(genecatlist, \(x){
#   ctlisttemp <- lapply(x, \(y){
#     cts[lapply(elrl, \(z) y %in% z) |> unlist()]
#   })
