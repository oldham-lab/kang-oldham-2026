...
load_dir <- file.path(file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_5/v2"))

### Plot largest network (consensus min, Jorstad)
mod_eig <- fread(data.table = F,file = file.path(load_dir, "Jorstad_DFC_consensusMin_noMerge_Modules/Pearson-no_TO_signum0.703_minSize3_merge_ME_1_1021/Module_eigengenes.csv")) |>
  column_to_rownames("Sample")
...
kme <- fread(data.table = F, file = file.path(load_dir, "Jorstad_DFC_consensusMin_noMerge_Modules/Pearson-no_TO_signum0.703_minSize3_merge_ME_1_1021/kME_table_.csv"))
...
mod_eig <- fread(data.table = F,file = file.path(load_dir, "Gabitto_DFC_consensusMin_noMerge_Modules/Pearson-no_TO_signum0.703_minSize3_merge_ME_1_1021/Module_eigengenes.csv")) |>
  column_to_rownames("Sample") |>
  select(jor_order)
rownames(mod_eig) <- class_info[,4]
colnames(mod_eig) <- 1:ncol(mod_eig)
...
kme <- fread(data.table = F, file = file.path(load_dir, "Gabitto_DFC_consensusMin_noMerge_Modules/Pearson-no_TO_signum0.703_minSize3_merge_ME_1_1021/kME_table_.csv"))
mod_fdr <- tapply(kme[,1], kme[,6], list) |>
  lapply(\(x) as.numeric(gsub("Mod", "", x)))
mod_fdr_gene <- lapply(mod_fdr, \(x) mod_seed[x] |> unlist() |> unique()) 
