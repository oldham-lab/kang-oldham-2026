...
parser.add_argument('--fig-width',  type=float, default=36.0, help='Figure width (in)')
parser.add_argument('--fig-height', type=float, default=10.0, help='Figure height (in)')
parser.add_argument('--coords-out', default=None,
    help='If set, save the SVG without tight-bbox and write data-anchor coordinates '
         '(svg size, px of data x=0 and x=10n, px of leaf baseline y=0) to this file, '
         'for compositing the dendrogram onto another figure.')
...
if args.coords_out:
    # Compositing mode: keep the y-axis, save WITHOUT tight-bbox (so data coords map
    # predictably onto the full SVG canvas), and export the anchor coordinates.
    ax.set_ylim(bottom=0)            # leaf baseline at axis bottom (no 5% negative margin)
    fig.patch.set_alpha(0.0)         # transparent bg so it overlays the panel cleanly
    ...
    d0 = ax.transData.transform((0, 0))            # data x=0  (left edge of leaf axis)
    d1 = ax.transData.transform((10 * n, 0))       # data x=10n (right edge)
    sx0 = d0[0] / fw_px * w_pt
    sx1 = d1[0] / fw_px * w_pt
    sy_leaf = h_pt * (1 - d0[1] / fh_px)           # leaf baseline (data y=0), top-down pt
    svg_file = OUTPUT_FILE if OUTPUT_FILE.endswith('.svg') else OUTPUT_FILE.replace('.pdf', '.svg')
    plt.savefig(svg_file, transparent=True)         # NO bbox_inches='tight'
    with open(args.coords_out, 'w') as f:
        f.write(f"svg_w={w_pt}\nsvg_h={h_pt}\nsx0={sx0}\nsx1={sx1}\nsy_leaf={sy_leaf}\n")
    print(f"Saved {svg_file} + coords {args.coords_out}")
else:
    plt.tight_layout()
