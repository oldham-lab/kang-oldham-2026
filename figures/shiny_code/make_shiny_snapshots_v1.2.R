# v1.2
# - Added Liu back
# - Standardized code with Fig. 3 and Fig. 4 code files
...
all_plots_full <- lapply(all_plots_full, \(x) x[ind_1016])
plots_bc_full <- lapply(plots_bc_full, \(x) x[ind_1016])
...
for(def in c("seed", 
  ...
  for(sum_type in c("mean", 
    ...
    for(d in 1:2){ # native log vs REI
      ...
      mitse <- lein_se[, match(allcts, colnames(mitse))]
      ...
      for(j in seq_along(all_plots[[1]])){
        rightside <- plot_grid(projplotlist[[these_mods[j]]], corlistind[[these_mods[j]]], nrow = 2, 
                               rel_heights = c(3, 1))
