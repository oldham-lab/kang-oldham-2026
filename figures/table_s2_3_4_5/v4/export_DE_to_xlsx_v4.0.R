# v4: regenerated from DE_v2.4 outputs (HGNC-canonicalized re-run, 2026-07-13);
# input paths repointed to in-repo fig_1/DE_v2.4. Otherwise identical to v3.
# v3: table titles, legend moved to first tab, rows sorted by celltype then adjusted
# p-value, all legend column names bolded (incl. FDR/padj), renamed output files
...
DE_DIR <- file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_1/DE_v2.4")

# edgeR results (.qs files — named lists of per-celltype data frames)
EDGER_JORSTAD_DFC  <- file.path(DE_DIR, "edgeR/jorstad/edgeR_jorstad_DFC_subclass_matchedGenes.qs")
EDGER_JORSTAD_MTG  <- file.path(DE_DIR, "edgeR/jorstad/edgeR_jorstad_MTG_subclass_matchedGenes.qs")
EDGER_SEAAD_DFC    <- file.path(DE_DIR, "edgeR/seaad/edgeR_seaad_DFC_subclass_matchedGenes.qs")
EDGER_SEAAD_MTG    <- file.path(DE_DIR, "edgeR/seaad/edgeR_seaad_MTG_subclass_matchedGenes.qs")

# DESeq2 results (.qs files — named lists of per-celltype data frames)
DESEQ2_JORSTAD_DFC <- file.path(DE_DIR, "DESeq2/jorstad/DESeq2_jorstad_DFC_subclass_matchedGenes.qs")
DESEQ2_JORSTAD_MTG <- file.path(DE_DIR, "DESeq2/jorstad/DESeq2_jorstad_MTG_subclass_matchedGenes.qs")
DESEQ2_SEAAD_DFC   <- file.path(DE_DIR, "DESeq2/seaad/DESeq2_seaad_DFC_subclass_matchedGenes.qs")
DESEQ2_SEAAD_MTG   <- file.path(DE_DIR, "DESeq2/seaad/DESeq2_seaad_MTG_subclass_matchedGenes.qs")
...
# Per-workbook supplementary table number and title (keyed by label)
TABLE_NUM <- c(
  jorstad_DFC = "S2",
  seaad_DFC   = "S3",
  jorstad_MTG = "S4",
...
TITLES <- c(
  jorstad_DFC = "Table S2 | Differential expression analysis of DFC subclasses from Jorstad et al.",
  seaad_DFC   = "Table S3 | Differential expression analysis of DFC subclasses from Gabitto et al.",
  jorstad_MTG = "Table S4 | Differential expression analysis of MTG subclasses from Jorstad et al.",
  seaad_MTG   = "Table S5 | Differential expression analysis of MTG subclasses from Gabitto et al."
)
...
# Rows are ordered by celltype (ascending), then by adjusted p-value (ascending).
compile_de_results <- function(reslist, padj_col) {
  out <- bind_rows(lapply(names(reslist), function(ct) {
    df <- reslist[[ct]]
  ...
  # Order rows by celltype (ascending), then adjusted p-value (ascending)
  out[order(out$celltype, out[[padj_col]]), , drop = FALSE]
}
...
format_legend_sheet <- function(wb, sheet, df, row_offset = 0) {
  n_cols <- ncol(df)
  n_rows <- nrow(df)
  ...
  addStyle(wb, sheet, header_style,
           rows = 1 + row_offset, cols = 1:n_cols, gridExpand = TRUE)
  # Bold the column-name cells through the final legend row (incl. FDR / padj)
  addStyle(wb, sheet, col_style,
           rows = (2 + row_offset):(n_rows + 2 + row_offset), cols = 1, gridExpand = TRUE)
  addStyle(wb, sheet, body_style,
           rows = (2 + row_offset):(n_rows + 2 + row_offset), cols = 2, gridExpand = TRUE)
...
build_workbook <- function(edger_path, deseq2_path, label) {
  ...
  writeData(wb, "Legend", EDGER_LEGEND, rowNames = FALSE, startRow = 2 + TITLE_OFFSET)
  format_legend_sheet(wb, "Legend", EDGER_LEGEND, row_offset = TITLE_OFFSET)

  deseq2_start <- nrow(EDGER_LEGEND) + 4 + TITLE_OFFSET
  ...
  # Bold the column-name cells through the final legend row (incl. padj)
  addStyle(wb, "Legend",
           createStyle(fontName = "Arial", fontSize = 11, textDecoration = "bold",
                       halign = "left", valign = "center", wrapText = TRUE),
           rows = (deseq2_start + 1):(deseq2_start + nrow(DESEQ2_LEGEND) + 1),
  ...
  out_path <- file.path(OUT_DIR, paste0("Kang_Table_", TABLE_NUM[[label]], "_", version, ".xlsx"))
