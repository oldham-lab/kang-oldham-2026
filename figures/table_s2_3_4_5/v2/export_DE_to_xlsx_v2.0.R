# added missing gene column to deseq tabs
...
compile_de_results <- function(reslist, padj_col) {
  bind_rows(lapply(names(reslist), function(ct) {
    ...
    df$celltype <- ct
    # Normalise gene column name to "Gene" (edgeR outputs "genes", DESeq2 uses rownames)
    if ("genes" %in% colnames(df)) colnames(df)[colnames(df) == "genes"] <- "Gene"
