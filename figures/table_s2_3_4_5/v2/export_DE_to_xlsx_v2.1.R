# fixed gene column in edgeR tab being entrez ids instead of gene symbols
...
# edgeR and DESeq2 have different output columns — define both
# ============================================================
...
# Round numeric columns to n significant figures
...
# Filter each celltype table to significant genes, add celltype column, concatenate.
# If gene names are stored in rownames (as in DESeq2 output), they are promoted to
# a "Gene" column placed immediately after "celltype".
compile_de_results <- function(reslist, padj_col) {
  bind_rows(lapply(names(reslist), function(ct) {
    df <- reslist[[ct]]
    # Promote rownames to Gene column if not already present as a column
    if (!"Gene" %in% colnames(df) && !is.null(rownames(df)) && !all(rownames(df) == seq_len(nrow(df)))) {
      df <- tibble::rownames_to_column(df, var = "Gene")
    }
    df <- df[!is.na(df[[padj_col]]) & df[[padj_col]] < PADJ_THRESH, ]
    df$celltype <- ct
    # edgeR stores gene info as a nested data frame in the "genes" column.
    # Extract the gene symbol sub-column (second column, which holds the symbol)
    # and replace the nested data frame with a plain character vector before renaming.
    if ("genes" %in% colnames(df)) {
      if (is.data.frame(df$genes)) {
        df$genes <- df$genes[[2]]  # second column is the gene symbol
      }
      colnames(df)[colnames(df) == "genes"] <- "Gene"
    }
    # Order: celltype, Gene, then all remaining columns
...
format_sheet <- function(wb, sheet, df) {
  ...
  # Header style: bold, centered, bottom border, light blue fill
...
# Apply consistent formatting to the Legend sheet
...
# Build a complete workbook for one dataset/region combination
build_workbook <- function(edger_path, deseq2_path, label) {
  ...
  # Section header for edgeR
  writeData(wb, "Legend", data.frame(x = "edgeR columns"), rowNames = FALSE, colNames = FALSE, startRow = 1)
  addStyle(wb, "Legend", createStyle(fontName = "Arial", fontSize = 11, textDecoration = "bold",
  ...
  # Section header for DESeq2 (below edgeR legend)
  deseq2_start <- nrow(EDGER_LEGEND) + 4
  writeData(wb, "Legend", data.frame(x = "DESeq2 columns"), rowNames = FALSE, colNames = FALSE,
            startRow = deseq2_start)
  addStyle(wb, "Legend",
  ...
  addStyle(wb, "Legend",
           createStyle(fontName = "Arial", fontSize = 11, halign = "left",
                       valign = "center", wrapText = TRUE),
           rows = (deseq2_start + 1):(deseq2_start + nrow(DESEQ2_LEGEND)),
           cols = 2, gridExpand = TRUE)

  # Note on filtering threshold
