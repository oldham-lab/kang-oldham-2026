...
EDGER_LEGEND <- data.frame(
  Column      = c("celltype", "genes", "logFC", "logCPM", "LR", "PValue", "FDR"),
  Description = c(
...
DESEQ2_LEGEND <- data.frame(
  Column      = c("celltype", "baseMean", "log2FoldChange", "lfcSE", "stat", "pvalue", "padj"),
  Description = c(
    "Cell subclass for which DE was tested (one-vs-all contrast)",
...
compile_de_results <- function(reslist, padj_col) {
  bind_rows(lapply(names(reslist), function(ct) {
    ...
    df$celltype <- ct
    # Move celltype to first column
    df[, c("celltype", setdiff(colnames(df), "celltype"))]
  }))
}
