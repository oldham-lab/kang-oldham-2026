...
#
# fig_8 v3: panel c restyled to match the newest Fig. S11 (fig_s11/v2) aesthetics —
# blue->red fill ramp, dashed-black diagonal (was a red outline), Arial font, and
# NEGLOG_CAP = 7. Data inputs are unchanged (fig_8's own DFC dCoPA gene list).
...
MIN_GENES   <- 5
FDR_THRESH  <- 0.05
NEGLOG_CAP  <- 7        # matches fig_s11/v2
...
RUNS <- list(
  ...
  list(
    ...
    save_dir      = file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_8/v3/"),
...
run_fisher_viz <- function(full_table,
  ...
  for (dir_name in c("lower")) {   # panel c is the "lower" panel (matches fig_s11/v2)
    panel_name <- dir_name
