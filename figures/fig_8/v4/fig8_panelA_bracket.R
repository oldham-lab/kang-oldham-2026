...
version <- "v4"   # v4 HGNC-corrected rebuild: outputs go to fig_8/v4
...
for(i in 1){
  ...
  # quit(save = "no", status = 0)   # HGNC rebuild: regen a-d (see stop before GSEA below)
...
# v4 HGNC rebuild: use the freshly-computed g_overlaps (from the HGNC-corrected v4
# dfc_overlaps above) for GSEA — do NOT reload the stale v1 dfc_overlaps.
...
source(file.path(save_dir, "scz_db.R"))
source(file.path(save_dir, "scz_ref_index.R"))
