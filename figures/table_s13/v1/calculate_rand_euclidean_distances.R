...
calculate_euclidean_distances <- function(real, # T or F
  ...
  if(real){
    ...
    proj_all <- lapply(unlist(snnames)[1:2], \(x){ # subclass only
      ...
      files1 <- files1[grep(x,files1)]
      if(length(files1) > 1){
        files1 <- files1[grep("topmodposbc_mean",files1)]
      }
      fread(data.table=F,file=files1)[these_mods,]
    })
  } else {
  ...
  # Save (subclass only)
  return(list(projdistpvalindiv[[1]], proj_dist_dir[[1]]))
