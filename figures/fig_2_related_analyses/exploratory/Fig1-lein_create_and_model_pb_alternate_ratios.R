...
###############################################
neu_glia_ratio <- list(c(.5,.5), # 50:50
                       c(.6, .4), # 60:40
                       c(.7, .3)) # 70:30
ratio_filename <- c("5050", "6040", "7030")

for(z in 1:3){ # for each neuron:glia ratio
  save_dirall <- paste0(file.path(Sys.getenv("MEGASET_DIR", "/home/gugene/RNAseq_megaset"), "13.1-sn_cell_pseudobulk/lein2023/DFC_indiv_donor_"), ratio_filename[z], "/")
  ...
  #for(i in seq_along(donorvec)){ # for each donor
    for(j in c(0)){
      ...
      # Assign celltype fractions for cell.frac
      cell_anno$ct_per <- NA
      cell_anno$ct_per[cell_anno$Class %in% c("Glutamatergic","GABAergic")] <- neu_glia_ratio[[z]][1]
      cell_anno$ct_per[cell_anno$Class=="Non-neuronal"] <- neu_glia_ratio[[z]][2]
