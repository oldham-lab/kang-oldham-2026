...
####################
save_dirall <- file.path(Sys.getenv("MEGASET_DIR", "/home/gugene/RNAseq_megaset"), "13.1-sn_cell_pseudobulk/lein2023/DFC_indiv_donor_ALRA/")
...
alraobj <- qread(file.path(Sys.getenv("DATA_OTHER_DIR", "/home/gugene/data_other"), "lein_dfc/impute/ALRA/lein_DFC_seurat_obj_with_ALRA_run_normalized.qs"))
cell_exprall <- as.matrix(alraobj@assays$alra@data)
rownames(cell_exprall) <- alraobj@assays$RNA@meta.features$feature_name
cell_exprall <- cell_exprall[, colnames(cell_exprall) %in% cell_annoall$Cell_ID]
cell_exprall <- cell_exprall[, match(cell_annoall$Cell_ID, colnames(cell_exprall))]
