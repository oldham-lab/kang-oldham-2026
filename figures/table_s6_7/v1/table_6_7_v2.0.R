...
# ============================================================
save_dir <- file.path(file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/table_s6_7/"))
...
write_legend_section <- function(wb, sheet, title, legend_df, start_row) {
  ...
  mergeCells(wb, sheet, cols = 1:2, rows = start_row)
  writeData(wb, sheet, x = title, startRow = start_row, startCol = 1, colNames = FALSE)
  addStyle(wb, sheet, section_style, rows = start_row, cols = 1:2, gridExpand = TRUE)
  ...
  addStyle(wb, sheet, subheader_style,
           rows = start_row + 1, cols = 1:2, gridExpand = TRUE)

  # Body rows
  col_name_style <- createStyle(
    fontName       = "Arial",
    fontSize       = 11,
    textDecoration = "bold",
    halign         = "left",
  ...
  addStyle(wb, sheet, col_name_style, rows = body_rows, cols = 1, gridExpand = TRUE)
  addStyle(wb, sheet, desc_style,     rows = body_rows, cols = 2, gridExpand = TRUE)
...
# ============================================================
build_workbook <- function(data, filename, dataset_label) {
