...
# ── SCZ gene database: genes in g_overlaps with documented SCZ associations ────
# Each entry: gene = list(n_refs = approx. no. relevant papers, ref = bibliography no.)
# Ordered alphabetically within each mechanistic block when displayed.
# Bibliography: gsea_summary_bibliography_dfc.md
library(gt)

scz_db <- list(
  # GABAergic interneuron dysfunction / neurodevelopmental connectivity
  CNTNAP2  = list(n=12, ref=1),
  # NMDAR hypofunction / postsynaptic density scaffolding
  DLG2     = list(n=8,  ref=2),
  # PI3K–Akt signalling dysregulation
  PIK3CB   = list(n=6,  ref=3),
  PIK3R1   = list(n=6,  ref=3),
  # Neuronal excitability / voltage-gated Na+ channel
  SCN2A    = list(n=15, ref=4),
  # Neurodevelopmental transcription / GABAergic maturation
  TCF4     = list(n=20, ref=5)
)
...
build_row <- function(ct) {
  ...
  # SCZ genes: match uppercase gene names against scz_db, sort alphabetically
  genes_upper <- toupper(genes)
  hits        <- genes[genes_upper %in% names(scz_db)]
  hits_up     <- genes_upper[genes_upper %in% names(scz_db)]

  if (length(hits) > 0) {
    ref_nums <- sapply(hits_up, function(g) scz_db[[g]]$ref)
    ord      <- order(hits_up)
    hits_s   <- hits[ord]
    refs_s   <- ref_nums[ord]
