...
OUT_DIR <- file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_1/DE_v2.2")
...
rawcounts_list <- future_lapply(
  list(
    jorstad = file.path(PB_DIR, "jorstad_cell_expression_by_donor_subclass_sum_DFC.csv"),
    sea     = file.path(PB_DIR, "seaad_cell_expression_by_donor_subclass_sum_DFC.csv")
  ),
...
cell_anno_pb <- list(
  "jorstad" = fread(
    file.path(PB_DIR, "jorstad_cell_annotations_by_donor_subclass_sum_DFC.csv"),
    data.table = FALSE) |>
    mutate(label     = colnames(rawcounts_list$jorstad),
           Cell_Type = factor(Cell_Type),
           Donor     = factor(Donor)),
  "sea" = fread(
    file.path(PB_DIR, "seaad_cell_annotations_by_donor_subclass_sum_DFC.csv"),
    data.table = FALSE) |>
...
if (!dir.exists(dirname(DESEQ2_JORSTAD_DFC))) dir.create(dirname(DESEQ2_JORSTAD_DFC), recursive = TRUE)
qsave(reslist_jorstad, file = DESEQ2_JORSTAD_DFC)
...
if (!dir.exists(dirname(DESEQ2_SEAAD_DFC))) dir.create(dirname(DESEQ2_SEAAD_DFC), recursive = TRUE)
qsave(reslist_sea, file = DESEQ2_SEAAD_DFC)
