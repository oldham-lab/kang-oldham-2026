# added variables for edgeR output folders
...
# --- edgeR output ---------------------------------------------------------
EDGER_JORSTAD_DE_DIR  <- file.path(Sys.getenv("MEGASET_DIR", "/home/gugene/RNAseq_megaset"), "14-ESSA_data/lein2023/data")
EDGER_SEAAD_DE_DIR_DFC <- file.path(Sys.getenv("MEGASET_DIR", "/home/gugene/RNAseq_megaset"), "14-ESSA_data/seaad2024_con/DE_by_subclass_Controls_jorstadGeneSubset/data")
EDGER_SEAAD_DE_DIR_MTG <- file.path(Sys.getenv("MEGASET_DIR", "/home/gugene/RNAseq_megaset"), "14-ESSA_data/seaad2024_con_mtg/DE_by_subclass_Controls_jorstadGeneSubset_mtg/data")

# --- DESeq2 output --------------------------------------------------------
DESEQ2_JORSTAD_OUT <- file.path(Sys.getenv("MEGASET_DIR", "/home/gugene/RNAseq_megaset"), "14-ESSA_data/lein2023/DEseq2/DEseq2_lein_dfc_subclass_geneSubset.qs")
DESEQ2_SEA_OUT     <- file.path(Sys.getenv("MEGASET_DIR", "/home/gugene/RNAseq_megaset"), "14-ESSA_data/seaad2024_con/DEseq2/DEseq2_seaad2024con_subclass_geneSubset_dfc.qs")
FIGURES_DIR        <- file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_1/v2")
...
future_lapply(REGIONS, function(i) {
  ...
  # Use data.table grouped sum — faster than dplyr::summarise(across(...))
  ...
  fwrite(data.frame(rawcounts),
         file      = file.path(JORSTAD_PB_DIR, paste0("Lein_2023_cell_expression_by_donor_subclass_sum_", i, ".csv")),
         row.names = TRUE)
  ...
  fwrite(new_sif[, -which(colnames(new_sif) == "unique")],
         file = file.path(JORSTAD_PB_DIR, paste0("Lein_2023_cell_annotations_by_donor_subclass_sum_", i, ".csv")))
...
# Helper: pseudobulk one SEAAD region and save expression + annotation files
pseudobulk_seaad <- function(expr_path, anno_path, region_tag) {
  ...
  expr_file <- file.path(SEAAD_PB_DIR,
                         paste0("seaad2024_cell_expression_by_donor_subclass_sum_controls", region_tag, ".csv"))
  anno_file <- file.path(SEAAD_PB_DIR,
                         paste0("seaad2024_cell_annotations_by_donor_subclass_sum_controls", region_tag, ".csv"))

  fwrite(data.frame(rawcounts), file = expr_file, row.names = TRUE)
  ...
  new_sif        <- new_sif[match(samp_lab, new_sif$label), ]
  fwrite(new_sif[, -which(colnames(new_sif) == "unique")], file = anno_file)

  message("SEAAD", region_tag, " pseudobulked")
}

# DFC
pseudobulk_seaad(SEAAD_EXPR_DFC, SEAAD_ANNO_DFC, region_tag = "")
# MTG
pseudobulk_seaad(SEAAD_EXPR_MTG, SEAAD_ANNO_MTG, region_tag = "_mtg")
...
sea_genes <- fread(
  file.path(SEAAD_PB_DIR, "seaad2024_cell_expression_by_donor_subclass_sum_controls.csv"),
  data.table = FALSE, select = 1L)[[1]]

future_lapply(REGIONS, function(i) {

  rawcounts    <- fread(
    file.path(JORSTAD_PB_DIR, paste0("Lein_2023_cell_expression_by_donor_subclass_sum_", i, ".csv")),
    data.table = FALSE)
  cell_anno_pb <- fread(
    file.path(JORSTAD_PB_DIR, paste0("Lein_2023_cell_annotations_by_donor_subclass_sum_", i, ".csv")),
    data.table = FALSE)
  ...
  calc_pseudobulk_DE(
    ...
    de_save_dir = EDGER_JORSTAD_DE_DIR
  )

  message("Jorstad ", i, " DE done")
}, future.seed = TRUE)
...
# Load Jorstad gene universe (from MTG file, used for subsetting in both regions)
jorstad_genes <- fread(
  file.path(JORSTAD_PB_DIR, "Lein_2023_cell_expression_by_donor_subclass_sum_MTG.csv"),
  data.table = FALSE, select = 1L)[[1]]

# DFC
seaad_rc_dfc   <- fread(file.path(SEAAD_PB_DIR, "seaad2024_cell_expression_by_donor_subclass_sum_controls.csv"),     data.table = FALSE)
seaad_anno_dfc <- fread(file.path(SEAAD_PB_DIR, "seaad2024_cell_annotations_by_donor_subclass_sum_controls.csv"),    data.table = FALSE)

seaad_rc_dfc <- seaad_rc_dfc[seaad_rc_dfc[, 1] %in% jorstad_genes, ]

calc_pseudobulk_DE(
  seaad_rc_dfc,
  save_string = "subclass_blockDonor_allgenes",
  celltype    = seaad_anno_dfc$Subclass,
  blocking    = seaad_anno_dfc[1],  # block by Donor ID
  de_save_dir = EDGER_SEAAD_DE_DIR_DFC
)
message("SEAAD DFC DE done")

# MTG
seaad_rc_mtg   <- fread(file.path(SEAAD_PB_DIR, "seaad2024_cell_expression_by_donor_subclass_sum_controls_mtg.csv"), data.table = FALSE)
seaad_anno_mtg <- fread(file.path(SEAAD_PB_DIR, "seaad2024_cell_annotations_by_donor_subclass_sum_controls_mtg.csv"), data.table = FALSE)
...
calc_pseudobulk_DE(
  ...
  de_save_dir = EDGER_SEAAD_DE_DIR_MTG
)
message("SEAAD MTG DE done")


# ============================================================
# STEP 5: DESeq2 — DFC, Jorstad vs SEAAD 2024
...
rawcounts_list <- future_lapply(
  list(
    jorstad = file.path(JORSTAD_PB_DIR, "Lein_2023_cell_expression_by_donor_subclass_sum_DFC.csv"),
    sea     = file.path(SEAAD_PB_DIR,   "seaad2024_cell_expression_by_donor_subclass_sum_controls.csv")
  ),
...
cell_anno_pb <- list(
  "jorstad" = fread(
    file.path(JORSTAD_PB_DIR, "Lein_2023_cell_annotations_by_donor_subclass_sum_DFC.csv"),
    data.table = FALSE) |>
    mutate(label     = colnames(rawcounts_list$jorstad),
           Cell_Type = factor(Cell_Type),
           Donor     = factor(Donor)),
  "sea" = fread(
    file.path(SEAAD_PB_DIR, "seaad2024_cell_annotations_by_donor_subclass_sum_controls.csv"),
    data.table = FALSE) |>
...
qsave(reslist_jorstad, file = DESEQ2_JORSTAD_OUT)
...
qsave(reslist_sea, file = DESEQ2_SEA_OUT)
