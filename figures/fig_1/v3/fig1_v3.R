# v3
# - same as v2, but loads the per-region gene-matched DE objects produced by
#   full_DE_pipeline_optimized_v2.4.R (DE_v2.4/) instead of generating DESeq2
#   inline and reading the old, un-matched edgeR objects.
...
# Cell-type list (subclass), taken from the Jorstad DFC pseudobulk annotation.
...
cts <- fread(file.path(Sys.getenv("DATA_OTHER_DIR", "/home/gugene/data_other"), "lein_dfc_pseudobulk_by_donor_all_genes/Lein_2023_cell_annotations_by_donor_subclass_sum_DFC.csv"), data.table = F)$Cell_Type |>
  factor() |>
  unique()
cts_upper <- as.character(cts)
cts_upper[c(13, 14, 18:24)] <- toupper(cts_upper[c(13, 14, 18:24)])

# Load DE objects (DFC) generated by full_DE_pipeline_optimized_v2.4.R
edger <- list(
  "jorstad" = qread(file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_1/DE_v2.4/edgeR/jorstad/edgeR_jorstad_DFC_subclass_matchedGenes.qs")),
  "sea" = qread(file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_1/DE_v2.4/edgeR/seaad/edgeR_seaad_DFC_subclass_matchedGenes.qs"))
)
...
reslist <- list(
  "jorstad" = qread(file = file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_1/DE_v2.4/DESeq2/jorstad/DESeq2_jorstad_DFC_subclass_matchedGenes.qs")),
  "sea" = qread(file = file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_1/DE_v2.4/DESeq2/seaad/DESeq2_seaad_DFC_subclass_matchedGenes.qs"))
) |>
  lapply(\(x){
    mapply(\(y,z){
      y$celltype <- z
...
# Save the full Panel D input: every shared marker per subclass with its mean rank,
# ordered by mean_marker_ranks (mean_marker_ranks is already sorted ascending, so the
# %in% filter preserves that order). This is the table data before the top-5 slice.
panel_D_data <- mapply(\(genes_shared, ranks_df, subclass){
  ranks_df[ranks_df$genes %in% genes_shared, ] |>
    mutate(Subclass = subclass) |>
    select(Subclass, genes, mean_ranks)
}, shared_markers, mean_marker_ranks, cts_upper, SIMPLIFY = F) |>
  do.call(what = "rbind")
fwrite(panel_D_data, file.path(save_dir, "panel_D_data.csv"))
...
# Non-neuronal   0.6868992
# GABAergic   0.9259636
# Glutamatergic   0.9497617
...
NUDGE <- list(
  E = list(Glutamatergic = c(0, 0), GABAergic = c(0, 2.7), `Non-neuronal` = c(0, 0.5), neuronal = c(0, 0)),
  F = list(Glutamatergic = c(0, 0), GABAergic = c(0, 2.7), `Non-neuronal` = c(0, 0), neuronal = c(0, 0))
)
