#!/usr/bin/env python3
"""
Assemble Figure 1 (v3) from its panels into PDF, PNG and PPTX -- de novo.

Replaces the media-swap approach of assemble_figure.py, which copied the
hand-laid-out fig_1/v2/v2.pptx and substituted its embedded images. That made a
superseded version directory a build input, and the template was never tracked,
so the figure could not be rebuilt from a clone.

Same house pattern as fig_7/v8 and fig_3/v4: compute placements once, emit the
PDF as vector, the PNG as a 300-dpi raster of it, and the PPTX with panels
rasterised at 300 dpi and the text live.

Two things the media-swap version needed and this one does not:
...
BASE = os.path.join(V3, "Kang_Figure_1_v3")
...
TITLE = ("Fig. 1 | Cell type marker genes in human neocortex vary by choice of "
         "study and algorithm")
...
# (text, x, y, w, centred?)
CAPTIONS = [
    ("Jorstad et al. 2023 (DFC, 3 donors)",  104.7,  36.0, 109.9, False),
    ("Gabitto et al. 2024 (DFC, 9 donors)",  104.5, 108.9, 109.6, False),
    ("Overlap of unique subclass markers",   399.5,  51.2, 126.0, True),
    ("Pairwise correlations of metacells \n(Jorstad et al. 2023, n = 28,676 genes)",
...
def read_gene_counts():
    d = {}
    try:
        for line in open(os.path.join(V3, GENE_COUNTS_FILE)):
            if "=" in line:
...
def compute_layout(pdf):
    ...
    try:
        eF, fF = ef_body_centers(os.path.join(V3, SRC["EF"]))
