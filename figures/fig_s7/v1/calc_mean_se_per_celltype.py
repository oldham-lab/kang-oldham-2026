"""
calc_mean_se_per_celltype.py

Calculates the mean and standard error (SE) of gene expression per cell type
for four AnnData objects, restricted to a specified subset of genes.
...
GENES = os.path.join(os.environ.get("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_s6/genes.txt")
# GENES = ["GENE1", "GENE2", "GENE3", ...]  # alternative: inline list

# Directory to write output CSV files
OUTPUT_DIR = os.path.join(os.environ.get("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_s7/v1")
...
def main():
    # -- Validate config ------------------------------------------------------
    if len(ADATA_PATHS) != 4:
        log.error("ADATA_PATHS must contain exactly 4 paths (got %d).", len(ADATA_PATHS))
        sys.exit(1)
    if len(DATASET_CONFIG) != 4:
        log.error("DATASET_CONFIG must contain exactly 4 entries (got %d).", len(DATASET_CONFIG))
        sys.exit(1)
    ...
    for i, (h5ad_path, cfg) in enumerate(zip(ADATA_PATHS, DATASET_CONFIG)):
        ...
        log.info("=" * 60)
        log.info("[%d/4] %s  |  filter=%s", i + 1, h5ad_path.name, cfg["filter"])
