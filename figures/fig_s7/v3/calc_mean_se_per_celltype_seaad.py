"""
calc_mean_se_per_celltype_seaad.py

Calculates the mean and standard error (SE) of gene expression per cell type
for the two SEA-AD 2024 AnnData objects, restricted to a specified subset of genes.

Counts are taken from adata.layers["UMIs"] and log1p-transformed before
computing statistics.

For each (adata, cell type) combination:
  1. Load raw UMI counts from adata.layers["UMIs"] and log1p-transform
  2. Subset cells to those matching the normal/control condition
  3. Subset the expression matrix to the genes of interest AND cells of that type
  4. Flatten (ravel) the resulting submatrix into a 1-D vector
  5. Compute mean and SE (Bessel-corrected: SE = SD / sqrt(n)) over all elements
...
ADATA_PATHS = [
  os.path.join(os.environ.get("DATA_DIR", "/mnt/bdata/gugene"), "datasets/SN_RNAseq/sea_ad_2024/RNAseq/SEAAD_A9_RNAseq_final-nuclei.2024-02-13.h5ad"),
...
DATASET_CONFIG = [
    {
        ...
        "filter":        True,
        "condition_col": "Overall AD neuropathological Change",
        "normal_value":  "Not AD",
    },
    {
        "celltype_col":  "Subclass",
...
def process_adata(adata, gene_subset: list, celltype_col: str,
    ...
    """
    Load UMI counts from adata.layers["UMIs"], log1p-transform, optionally
    filter to normal samples, then compute mean and SE per cell type.
    ...
    """
    # -- Load UMI counts and log1p-transform ----------------------------------
    if "UMIs" not in adata.layers:
        log.error("  'UMIs' layer not found. Available layers: %s", list(adata.layers.keys()))
        sys.exit(1)
    Xu = adata.layers["UMIs"].copy()
    if scipy.sparse.issparse(Xu):
        Xu = Xu.astype(np.float64)
        Xu.data = np.log1p(Xu.data)
    else:
        Xu = np.log1p(np.asarray(Xu, dtype=np.float64))
    adata.X = Xu
    log.info("  Set adata.X from layers['UMIs'] and applied log1p.")
    ...
    log.info("  Expression matrix: %d cells x %d genes", X.shape[0], X.shape[1])

    # -- Select common-universe genes (canonical HGNC match) ------------------
    adata_genes   = list(var.index)
    gene_idx, present_genes = select_common_indices(adata_genes, gene_subset)
    missing       = len(gene_subset) - len(present_genes)
    if missing:
        log.warning("  %d / %d common-universe genes not found in this adata; skipped.",
