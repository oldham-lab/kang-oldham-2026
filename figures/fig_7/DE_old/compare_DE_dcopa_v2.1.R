...
get_de_for_ct <- function(dcopa_ct, de_list, use_mapping) {
  if (use_mapping && dcopa_ct %in% names(dcopa_to_de)) {
    ...
    parts <- lapply(mapped_cts, function(de_ct) de_list[[de_ct]])
    parts <- Filter(Negate(is.null), parts)
    if (length(parts) == 0) return(NULL)
    # Pool genes across mapped celltypes; for genes appearing in multiple
    # celltypes keep the row with the lowest FDR (logFC sign is preserved)
