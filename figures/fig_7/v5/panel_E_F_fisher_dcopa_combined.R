...
RUNS <- list(
  ...
  list(
    dcopa_path    = file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_7/v5/panel_E_DFC_dcopa_genelist.csv"),
    save_dir      = file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_7/v5"),
    universe_path = file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_7/v3/all_bulk_genes.txt"),
    file_prefix   = "panel_E_DFC"
    # Optional: override axis labels (omit to use set_keys as axis text)
    # axis_label    = c(
    #   "Gabitto + Liu, DFC + MTG"             = "Gabitto + Liu\nDFC + MTG",
    #   "Gabitto + Liu, DFC + MTG (AD modules)" = "Gabitto + Liu, DFC + MTG\n(AD modules)"
    # )
  ),
    list(
    dcopa_path    = file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_7/v5/panel_E_MTG_dcopa_genelist.csv"),
    save_dir      = file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_7/v5"),
    universe_path = file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_7/v3/all_bulk_genes.txt"),
    file_prefix   = "panel_E_MTG"
    # Optional: override axis labels (omit to use set_keys as axis text)
    # axis_label    = c(
    #   "Gabitto + Liu, DFC + MTG"             = "Gabitto + Liu\nDFC + MTG",
    #   "Gabitto + Liu, DFC + MTG (AD modules)" = "Gabitto + Liu, DFC + MTG\n(AD modules)"
...
run_fisher_analysis <- function(dcopa_path,
  ...
  summary_table <- full_table |>
    dplyr::group_by(Group1, Group2, Celltype1, Celltype2) |>
    dplyr::summarise(
      n_tests              = dplyr::n(),
  ...
  fwrite(full_table,    file = file.path(save_dir, paste0(file_prefix, "_fisher_dcopa_full.csv")))
  fwrite(summary_table, file = file.path(save_dir, paste0(file_prefix, "_fisher_dcopa_summary.csv")))
  message("Saved CSVs to: ", save_dir)
...
run_fisher_viz <- function(full_table,
  ...
  all_cts  <- union(unique(ft$Celltype1), unique(ft$Celltype2))
  ct_order <- c(intersect(ct_order_requested, all_cts),
                setdiff(all_cts, ct_order_requested))
  ...
  build_ct_heatmap <- function(df, sig_pts, na_pts,
    ...
    ggplot(df, aes(x = ct2, y = ct1)) +
      ...
      theme(
        ...
        axis.title.x    = element_text(size = 28, face = "plain", margin = margin(t = 6)),
        axis.title.y    = element_text(size = 28, face = "plain", margin = margin(r = 6)),
  ...
  for (dir_name in c("higher", "lower")) {
    panel_name <- if (dir_name == "higher") "F" else "E"
    ...
    p <- build_ct_heatmap(df_dir, sig_dir, na_dir, g1_label, g2_label)
    save_plot(file.path(save_dir, paste0(file_prefix, "_panel_", panel_name)),
              p, plot_w, plot_h)
    ...
    if (dir_name == "higher") p_higher <- p else p_lower <- p
  }

  p_for_legend <- if (!is.null(p_higher)) p_higher else p_lower
