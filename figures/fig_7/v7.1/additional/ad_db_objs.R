...
#                 gsea_summary_bibliography_CTRLmods_mtg.md (refs 1–23)
...
ad_db_CTRLmods_dfc <- list(
  ...
  # Autophagy-lysosome pathway
  ATG7    = list(n = 31, ref = 1),   # neuronal ATG7 KO causes axonal swellings and neurodegeneration (Komatsu 2006)
...
ad_db_CTRLmods_mtg <- list(
  ...
  SYNJ1    = list(n = 18, ref = 19),  # PI(4,5)P2 phosphatase; triplication enlarges endosomes and increases Aβ (Cossec 2012)

  # Ca2+ dysregulation
  PPP3CB   = list(n = 16, ref = 13),  # Aβ-activated calcineurin drives dendritic spine loss (Wu 2010)
  STIM2    = list(n = 19, ref = 18),  # STIM2 loss reduces SOCE and causes mushroom-spine loss in AD (Zhang 2015)

  # Endosomal-lysosomal trafficking
  PICALM   = list(n = 44, ref = 11),  # GWAS AD risk gene; regulates clathrin-mediated APP endocytosis and Aβ autophagy (Harold 2009)
  SORL1    = list(n = 41, ref = 17),  # SorLA sorting receptor; reduced in AD; SORL1 variants confer AD risk (Rogaeva 2007)
  VPS35    = list(n = 27, ref = 23),  # retromer subunit; reduced in AD; loss increases amyloidogenic APP processing (Small 2005)
  ...
  VDAC1    = list(n = 29, ref = 22),  # APP physically arrests in VDAC1-containing import channel (Anandatheerthavarada 2003)
  ...
  # Synaptic failure / neurotrophin signalling
  NTRK2    = list(n = 42, ref = 10),  # TrkB BDNF receptor; BDNF delivery rescues synaptic loss and memory in AD models (Nagahara 2009)
  SNAP25   = list(n = 53, ref = 16),  # presynaptic SNARE; SNAP25 reduced early during AD progression (Masliah 2001)

  # Tau phosphorylation / dephosphorylation
  MAPK8    = list(n = 41, ref = 6),   # JNK1/MAPK8 activated in AD neurons; phosphorylates tau (Zhu 2001)
  PPP2CA   = list(n = 56, ref = 12),  # PP2A catalytic subunit; major tau phosphatase; PP2A activity reduced in AD brain (Gong 1995)

  # PI3K / mTOR / PTEN pathway
  PTEN     = list(n = 34, ref = 15),  # PTEN loss in AD neurons unleashes PI3K/AKT/mTOR, suppressing autophagy (Griffin 2005)

  # UPS dysfunction
  PSMA5    = list(n = 9,  ref = 14),  # 20S α-subunit; proteasome activity reduced in AD (Keller 2000)
  PSMD11   = list(n = 6,  ref = 14),  # 19S subunit; contributes to UPS dysfunction in AD (Keller 2000)
  PSMD5    = list(n = 7,  ref = 14),  # 19S subunit; 19S function impaired in AD (Keller 2000)
  UBQLN1   = list(n = 19, ref = 20),  # presenilin-interacting UBL-UBA shuttle; loss disrupts presenilin and UPS in AD (Mah 2000)
  USP14    = list(n = 19, ref = 21),  # proteasomal DUB; USP14 inhibition restores tau clearance in AD model neurons (Lee 2010)
