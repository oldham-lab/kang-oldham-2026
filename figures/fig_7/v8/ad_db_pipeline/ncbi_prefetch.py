#!/usr/bin/env python3
"""Phase 1 (deterministic, no model): fetch NCBI PubMed data for the fig_7 lit-review genes.
Per prompt_ad_db_v3.txt Agent-1 search rules: query '"GENE"[TIAB] AND "Alzheimer"[TIAB]',
2000-present, top 5 by relevance; also capture total hit count (Step 4.5). One esearch +
one batched efetch per gene => real PMIDs by construction (no hallucination possible).
Paced at ~2.5 req/s (no API key), retry/backoff, incremental checkpoint to ncbi_cache.json."""
...
GENES = os.path.join(SC, "genes_unique_pipeline.txt")
CACHE = os.path.join(SC, "ncbi_cache.json")
...
def main():
    ...
    print(f"DONE. {len(cache)} genes cached; {n_hits} have >=1 abstract; "
          f"{len(cache)-n_hits} have 0 AD hits.", flush=True)
