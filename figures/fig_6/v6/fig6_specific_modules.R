...
context <- if(length(args) >= 3 && nzchar(args[3])) args[3] else "AD"
# ---------------------------------------------------------

save_dir <- file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/fig_6/v6")
...
target_mods     <- if(length(args) >= 1 && nzchar(args[1])) as.numeric(strsplit(args[1], ",")[[1]]) else c(83, 147)
...
highlight_genes <- if(length(args) >= 4 && nzchar(args[4])) strsplit(args[4], ",")[[1]]            else c("TMEM106B")
