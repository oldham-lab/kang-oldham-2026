...
# ============================================================
INPUT_CSV <- file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/table_s12/v1/table_s12.csv")   # update to actual path
OUT_PATH  <- file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "figures/table_s12/v1/Kang_Table_S12_v1.xlsx")  # update to actual path
...
writeData(wb, "Table S12", df, rowNames = FALSE)
...
addStyle(wb, "Table S12", header_style,
         rows = 1, cols = 1:n_cols, gridExpand = TRUE)
addStyle(wb, "Table S12", body_style,
...
setColWidths(wb, "Table S12", cols = 1:n_cols, widths = col_widths + 2)
freezePane(wb, "Table S12", firstRow = TRUE)
...
# Title row
writeData(wb, "Legend",
          data.frame(x = "Table S12 | Assignments of all genes to all modules identified by CoPA in frontal cortex from patients with Alzheimer's disease"),
