...
# # Load dataset (Lein MTG) from mtx and convert to anndata object with genes and barcodes
# scipy_csc_matrix = mmread("/home/gugene/bdata/@shared/scsn.expr_data/human_expr/postnatal/jorstad_2023_PMID_37824655/10x//expression_MTG/matrix.mtx")
# if not isinstance(scipy_csc_matrix, csc_matrix):
#     scipy_csc_matrix = scipy_csc_matrix.tocsc()
# adata = ad.AnnData(scipy_csc_matrix.T)
# genetsv = pd.read_csv("/home/gugene/bdata/@shared/scsn.expr_data/human_expr/postnatal/jorstad_2023_PMID_37824655/10x/expression_MTG/genes.tsv", sep='\t', header=None)
# genetsv.columns = ['gene_names']
# genetsv = genetsv.set_index('gene_names', drop=False)
# sn_anno = pd.read_csv("/mnt/bdata/@shared/scsn.expr_data/human_expr/postnatal/jorstad_2023_PMID_37824655/10x/author_barcode_annotations_MTG.csv")
# sn_anno = sn_anno.set_index('Cell_ID', drop=False)
# adata.obs = sn_anno
# adata.var = genetsv
# adata.raw = adata
# sc.pp.log1p(adata)
# adata.write(filename="/home/gugene/bdata/@shared/scsn.expr_data/human_expr/postnatal/jorstad_2023_PMID_37824655/10x//expression_MTG.h5ad")
...
# Read data (Lein MTG)
adata = ad.read_h5ad(os.path.join(os.environ.get("SHARED_DATA_DIR", "/mnt/bdata/@shared"), "scsn.expr_data/human_expr/postnatal/jorstad_2023_PMID_37824655/10x/expression_MTG.h5ad"))
...
combdf.to_csv(os.path.join(os.environ.get("DATA_DIR", "/mnt/bdata/gugene"), "data/greedy_march_pipeline_output/finalNonNorm_minsize10_unmerged/LeinMTG/sn_proj_indices/log_native/indices_se_Cell_Type.csv"), index = False)
...
combdf.columns = adata.obs["Cell_Type"].unique()
combdf = combdf.sort_index(axis=1)
combdf.to_csv(os.path.join(os.environ.get("DATA_DIR", "/mnt/bdata/gugene"), "data/greedy_march_pipeline_output/finalNonNorm_minsize10_unmerged/LeinMTG/sn_proj_indices/log_normByMean/indices_se_Cell_Type.csv"), index = False)
...
combdf.columns = adata.obs["Cell_Type"].unique()
combdf = combdf.sort_index(axis=1)
combdf.to_csv(os.path.join(os.environ.get("DATA_DIR", "/mnt/bdata/gugene"), "data/greedy_march_pipeline_output/finalNonNorm_minsize10_unmerged/LeinMTG/sn_proj_indices/log_REI/indices_se_Cell_Type.csv"), index = False)
