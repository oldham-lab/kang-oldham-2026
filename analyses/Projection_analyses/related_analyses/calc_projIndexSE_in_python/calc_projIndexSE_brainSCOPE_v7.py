...
def make_wide_table(long_df: pd.DataFrame, cohort: str, disorder: str) -> pd.DataFrame:
    ...
    # Reindex to the full module list so modules with no data in this
    # cohort/disorder still appear as NaN rows rather than being silently dropped
    all_modules = sorted(long_df["Module"].unique(), key=module_sort_key)
    wide = wide.reindex(all_modules)
...
def main(argv=None):
    ...
    for cohort in sorted(ALLOWED_COHORTS):
        for disorder in sorted(ALLOWED_DISORDERS):
            ...
            wide.to_csv(out_path, float_format="%.6g", index=False)
