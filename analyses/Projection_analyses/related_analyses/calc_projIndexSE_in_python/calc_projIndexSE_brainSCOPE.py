...
(cells) sharing the same Disorder, CellType, and gene Module, pooled across
the CMC and SZBDMulti-Seq cohorts.

Two output tables are produced (one per disorder: control and Schizophrenia):
  Rows    = gene modules  (keys of the module JSON file)
  Columns = cell types    (derived from column headers of each matrix)
  Values  = SE of expression across all cells in that (disorder, celltype, module) group
...
Output files (written to --output_dir):
  control_SE.tsv        -- rows: modules, columns: cell types
  Schizophrenia_SE.tsv  -- rows: modules, columns: cell types
...
-----
  ...
  - Each (gene, cell) pair within a module is treated as an individual
    ...
    values pooled over every sample in the (disorder, celltype) group.
...
def parse_celltype(barcode: str, sep: str = "_") -> str:
    ...
    Column format: <barcode>_<CellType>  e.g. AAACCCAAGCATCACG-1_Ex-L2-3
    Everything after the first occurrence of `sep` is the cell-type label.
    """
    idx = barcode.find(sep)
    return barcode[idx + 1:] if idx != -1 else "Unknown"
...
class StreamingStats:
    ...
    calculation, keyed by (disorder, celltype, module).
    ...
    def __init__(self):
        # key: (disorder, celltype, module_name)
    ...
    def add_sample(
        ...
        For each (disorder, celltype, module) group the gene × cell
        ...
        for col in df.columns:
            ct_groups[parse_celltype(col, celltype_sep)].append(col)
    ...
    def compute_se(self) -> pd.DataFrame:
        ...
        Finalize accumulated statistics and return a long-form DataFrame:
          Disorder | CellType | Module | N_observations | Mean | SD | SE
        ...
        """
        rows = []
        for (disorder, ct, mod), entry in self._data.items():
            n = entry["n"]
            if n < 2:
                log.warning(
                    "Group (%s, %s, %s) has only n=%d observation; SE set to NaN.",
                    disorder, ct, mod, n,
                )
                rows.append((disorder, ct, mod, n, np.nan, np.nan, np.nan))
                continue
            ...
            sd   = np.sqrt(var)
            se   = sd / np.sqrt(n)
            rows.append((disorder, ct, mod, n, mean, sd, se))

        return pd.DataFrame(
            rows,
            columns=["Disorder", "CellType", "Module",
...
def make_wide_table(long_df: pd.DataFrame, disorder: str) -> pd.DataFrame:
    """
    Pivot the long SE table for one disorder into a wide matrix:
      Index   = Module  (rows)
      Columns = CellType
      Values  = SE
    """
    subset = long_df[long_df["Disorder"] == disorder][["Module", "CellType", "SE"]]
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description=(
            "Compute module-level SE of expression per (Disorder x CellType), "
            "pooled across CMC and SZBDMulti-Seq cohorts. "
            "Outputs two TSV tables: control_SE.tsv and Schizophrenia_SE.tsv."
        ),
...
def main(argv=None):
    ...
    # ------------------------------------------------------------------
    # 4. Compute SE and write wide output tables
    # ------------------------------------------------------------------
    ...
    for disorder in sorted(ALLOWED_DISORDERS):
        wide = make_wide_table(long_df, disorder)
        if wide.empty:
            log.warning("No data accumulated for disorder '%s'; skipping output.", disorder)
            continue

        out_path = out_dir / f"{disorder}_SE.tsv"
        wide.to_csv(out_path, sep="\t", float_format="%.6g")
        output_files.append(str(out_path))
        log.info(
            "  Wrote %-40s  (%d modules x %d cell types)",
    ...
    summary = (
        long_df.groupby(["Disorder", "CellType"])
        .agg(N_modules=("Module", "count"), Total_observations=("N_observations", "sum"))
        .reset_index()
    )
    print("\n=== Summary: observations and modules per (Disorder x CellType) ===")
    print("(N_observations = total gene x cell pairs accumulated per group)")
    print(summary.to_string(index=False))
    if n_null:
        print(f"\nWARNING: {n_null} (disorder, celltype, module) groups had n<2 and SE=NaN.")
