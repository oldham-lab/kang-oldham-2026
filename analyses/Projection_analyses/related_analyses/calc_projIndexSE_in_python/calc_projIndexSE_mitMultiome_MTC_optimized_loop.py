...
sc.pp.log1p(adata, copy=False, chunked=False)  # Warning about already log-transformed is a false alarm;
                                                ...
                                                # Deleting 'log1p' unstructured object removes the warning.
...
#   cc_col_map         : dict mapping adata.obs column values -> group label
runs = [
    {
        "case_control_names": ["Con", "AD"],
        "out_dir": os.path.join(os.environ.get("DATA_DIR", "/mnt/bdata/gugene"), "data/greedy_march_pipeline_output/finalNonNorm_minsize10_unmerged/MIT_ADMultiome_AllADVsCon_MTC/"),
        "cc_col_map": {
            "source_col": "Pathology",
            "mapping": {
                "nonAD": "Con",
            },
            "default": "AD",   # anything not in mapping and not excluded gets this label
            "exclude": []
        }
    },
    {
        "case_control_names": ["Con", "Early"],
        "out_dir": os.path.join(os.environ.get("DATA_DIR", "/mnt/bdata/gugene"), "data/greedy_march_pipeline_output/finalNonNorm_minsize10_unmerged/MIT_ADMultiome_earlyVsCon_MTC/"),
        "cc_col_map": {
            "source_col": "Pathology",
            "mapping": {
                "nonAD": "Con",
            ...
            "default": None,   # anything not in mapping and not excluded gets this label
            "exclude": []
        }
    },
    {
        "case_control_names": ["Late", "Early"],
        "out_dir": os.path.join(os.environ.get("DATA_DIR", "/mnt/bdata/gugene"), "data/greedy_march_pipeline_output/finalNonNorm_minsize10_unmerged/MIT_ADMultiome_lateVsEarly_MTC/"),
        "cc_col_map": {
            "source_col": "Pathology",
            "mapping": {
                "lateAD": "Late",
...
for run in runs:
    ...
    for c in case_control_names:
        ...
        for varlist, subfolder in [
            ...
            fname = f"{out_dir}/sn_proj_indices/{subfolder}/indices_se_{ct_string}{c}.csv"
