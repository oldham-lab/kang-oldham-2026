...
if download_MSBB:
    if not download_all:
        for file in syn.getChildren('syn8612191'):
            if file['name'].startswith('hB'):
               # synapseutils.syncFromSynapse(syn, file['id'], path = 'fastq/')
                print(file['id'], file['name'])
    else:
        # fastq files
        files = synapseutils.syncFromSynapse(syn, 'syn8612191', path = 'fastq/')
...
if download_MAYOTCX:
     ...
     # metadata MAYO
     files = synapseutils.syncFromSynapse(syn, 'syn11384571', path = 'metadata/') # AMP-AD_Mayo_WGS_covariates.csv

     files = synapseutils.syncFromSynapse(syn, 'syn5223705', path = 'metadata/') # MayoRNAseq_RNAseq_CBE_covariates.csv

     files = synapseutils.syncFromSynapse(syn, 'syn3817650', path = 'metadata/') # MayoRNAseq_RNAseq_TCX_covariates.csv
...
if download_ROSMAP:
    ...
    if not download_all:
        ...
        # fastq
        files = synapseutils.syncFromSynapse(syn, 'syn8612097', path = 'fastq')
        # ROSMAP metadata
        files = synapseutils.syncFromSynapse(syn, 'syn3157322', path = 'metadata')
        files = synapseutils.syncFromSynapse(syn, 'syn11958660', path = 'metadata') #ROSMAP WGS
        files = synapseutils.syncFromSynapse(syn, 'syn11384589', path = 'metadata') #ROSMAP WGS
