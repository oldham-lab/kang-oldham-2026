cd ${DATA_DIR:-/mnt/bdata/gugene}/datasets/RNAseq/CMC/RNAseq/
...
sudo docker pull broadinstitute/gtex_rnaseq:V10

path_to_data=${DATA_DIR:-/mnt/bdata/gugene}/datasets/RNAseq/CMC/RNAseq/

for input_bam in *.bam; do
sample_id=$(echo $input_bam | sed "s/.accepted_hits.sort.coord.bam/.fastq/")
sudo docker run --rm -v $path_to_data:/data -t broadinstitute/gtex_rnaseq \
...
cd ${DATA_DIR:-/mnt/bdata/gugene}/datasets/RNAseq/CMC/RNAseq/fastq/
