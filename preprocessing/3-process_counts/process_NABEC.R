...
dir <- file.path(Sys.getenv("DATA_DIR", "/mnt/bdata/gugene"), "datasets/RNAseq/NABEC/fastq/trimmed/kal_quant/")
...
samp_names <- gsub("_trimmed_transcripts_quant_kal", "", samp_names)
colnames(expr) <- samp_names
expr <- cbind(rownames(expr), data.frame(expr))
colnames(expr)[1] <- "gene_id"
fwrite(expr, file = file.path(Sys.getenv("DATA_DIR", "/mnt/bdata/gugene"), "datasets/RNAseq/count_mats/expr_NABEC_raw_counts.csv"))
...
expr <- fread(file.path(Sys.getenv("DATA_DIR", "/mnt/bdata/gugene"), "datasets/RNAseq/count_mats/expr_NABEC_raw_counts.csv"), data.table=F)
colnames(expr)[1] <- "Gene"
sif <- fread(file.path(Sys.getenv("REPO_DIR", "/home/gugene/code/git/kang-oldham-2026"), "RNAseq_preprocessing/sample_info_files/sif_NABEC.csv"), data.table=F)
...
# In the NABEC dataset there are 69 unique individuals, 69 samples
...
SampleNetwork(
  ...
  subgroup1=33, # color samples by sex
  sampleinfo1=sif,
  samplelabels1=1,
...
expr <- fread(file.path(Sys.getenv("DATA_DIR", "/mnt/bdata/gugene"), "datasets/RNAseq/count_mats/NABEC_samp_filt_SampleNetworks/1_04-53-24/NABEC_samp_filt_1_69_outliers_removed.csv"), data.table=F)
...
fwrite(expr2, file = file.path(Sys.getenv("DATA_DIR", "/mnt/bdata/gugene"), "datasets/RNAseq/processed_mats/expr_NABEC_ctf_asinh.csv"))
